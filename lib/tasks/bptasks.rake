    ...
    ldap.search(base: treebase, attributes: attrs, filter: filter) do |entry|
      ...
      if (uac & 2 == 0) || !entry['mail'].first.to_s.empty? # пользователь не заблокирован и имеет не пустой e-mail
        ...
        if usr.new_record?
          ...
          usr.update_attribute(:firstname, firstname) unless usr.firstname == firstname
          usr.update_attribute(:lastname, lastname) unless usr.lastname == lastname
          usr.update_attribute(:middlename, middlename) unless usr.middlename == middlename
          usr.update_attribute(:displayname, displayname) unless usr.displayname == displayname
          ...
          usr.update_attribute(:phone, phone) unless usr.phone == phone # телефон
          usr.update_attribute(:office, office) unless usr.office == office # офис
          usr.update_attribute(:middlename, middlename) unless usr.middlename == middlename # отчество
